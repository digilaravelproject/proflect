                ...
                <div class="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-10 lg:px-8">
                    ...
                    <div class="mt-10 w-full rounded-3xl border border-white/10 bg-black/60 p-6 sm:p-8 lg:p-10 shadow-xl backdrop-blur">
                        <div class="grid gap-10 lg:grid-cols-2">
                            <div class="space-y-6">
                                <h2 class="text-4xl font-black tracking-tight text-white lg:text-5xl">VERIFY WARRANTY</h2>
                                <p class="text-lg text-gray-200">A verification code was sent to <strong>{{ $warranty->email }}</strong> for warranty number <strong>{{ $warranty->unique_number }}</strong>.</p>
                                <p class="text-gray-300">Enter the 4-digit code from your email to see your warranty card.</p>

                                <p class="text-sm text-gray-400">
                                    Didn’t get a code? <a href="{{ route('warranty.check') }}" class="text-yellow-300 hover:text-yellow-200">Request a new code</a>.
                                </p>
                            </div>

                            <div class="rounded-2xl border border-white/10 bg-white/5 p-6 shadow-xl backdrop-blur">
                                @if (session('otp_sent'))
                                    <div class="mb-4 rounded-lg border border-blue-500 bg-blue-900/40 px-4 py-3 text-sm text-blue-100">
                                ...
                                <form action="{{ route('warranty.check.verify.post', ['warranty' => $warranty->id]) }}" method="POST" class="space-y-4" id="otpForm">
                                    @csrf

                                    <div>
                                        <p class="text-sm text-gray-300">Enter verification code</p>
                                        <div class="mt-2 flex items-center justify-center gap-2">
                                            @for ($i = 0; $i < 4; $i++)
                                                <input
                                                    ...
                                                    inputmode="numeric"
                                                    maxlength="1"
                                                    class="otp-digit w-12 rounded-lg border border-white/15 bg-white/5 px-2 py-3 text-center text-xl font-semibold text-white placeholder:text-white/35 focus:border-yellow-300 focus:outline-none focus:ring-2 focus:ring-yellow-300"
                                                />
                                            @endfor
                                        </div>
                                    </div>
        ...
        <script>
            ...
            if (otpDigits.length) {
                ...
                function updateHiddenOtp() {
                    otpHidden.value = otpDigits.map((i) => i.value).join('');
                }
