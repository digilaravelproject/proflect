...
class WarrantyController extends Controller
{
    ...
    public function validateCode(Request $request)
    {
        ...
        $code = WarrantyCode::where('code', $data['unique_number'])->first();

        if (! $code) {
            return response()->json([
                'valid' => false,
                'message' => 'This code does not exist or was not generated by admin. Please enter a valid admin code.',
                'status' => 'not_found',
            ]);
        }
        ...
        if ($code->status === 'used') {
            return response()->json([
                'valid' => false,
                'message' => 'This code has already been used. Please enter a different code or contact support.',
                'status' => 'used',
            ]);
        }
        ...
        return response()->json([
            'valid' => true,
            'message' => 'This code is available and ready for use.',
            'status' => 'available',
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            $warranty = DB::transaction(function () use ($data, $todays_datetime) {
                ...
                $warrantyCode = WarrantyCode::where('code', $uniqueNumber)->lockForUpdate()->first();

                if ($warrantyCode && $warrantyCode->status === 'used') {
                    throw new \RuntimeException('This code has already been used. Please choose a different warranty number.');
                }

                if (! $warrantyCode) {
                    throw new \RuntimeException('This code was not generated by admin. Please enter a valid admin code.');
                }

                $data['unique_number'] = str_pad($uniqueNumber, 5, '0', STR_PAD_LEFT);
                ...
                $warrantyCode->update([
                    'owner_name' => $warrantyCode->owner_name ?: $warranty->name,
                    'owner_email' => $warranty->email,
                    'status' => 'used',
                    'used_at' => now(),
            ...
            return back()->withErrors(['unique_number' => $e->getMessage()])->withInput();
        }
